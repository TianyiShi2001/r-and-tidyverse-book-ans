...
\hypertarget{ch1}{%
\chapter{R和RStudio介绍和安装教程}\label{ch1}}
...
其中\texttt{2}是计算结果， \texttt{{[}1{]}}我们在下一章再解释。\texttt{\textgreater{}\ 1\ +\ 1}是input，\texttt{{[}1{]}\ 2}是output.
...
这小节包含了一些不重要的内容，因此\textbf{可酌情跳到下一章（第@\ref(vectors-logicals-and-functions)章。}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{class}\NormalTok{(nums)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{class}\NormalTok{(}\KeywordTok{c}\NormalTok{(}\DecValTok{3}\OperatorTok{+}\NormalTok{5i, 3i, }\DecValTok{1}\OperatorTok{+}\NormalTok{0i))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{#没必要赋值一个变量；c(...)的计算结果就是一个向量，并直接传给`class()`函数}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{length}\NormalTok{(}\KeywordTok{c}\NormalTok{(}\StringTok{"Guten Morgen"}\NormalTok{, }\StringTok{""}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\hypertarget{consoler-script}{%
\subsection{Console和R script编辑器的一些特性}\label{consoler-script}}
...
\begin{Shaded}
\begin{Highlighting}[]
\ControlFlowTok{if}\NormalTok{(}\DecValTok{1} \OperatorTok{+}\StringTok{ }\DecValTok{1} \OperatorTok{==}\StringTok{ }\DecValTok{2} \OperatorTok{&}\StringTok{ }\DecValTok{1} \OperatorTok{+}\StringTok{ }\DecValTok{2} \OperatorTok{==}\StringTok{ }\DecValTok{5}\NormalTok{)\{}
...
\includegraphics{R与tidyverse——数据分析入门_files/figure-latex/unnamed-chunk-52-1.pdf}
...
\NormalTok{X_squared <-}\StringTok{ }\KeywordTok{matrix}\NormalTok{(}\KeywordTok{rep}\NormalTok{(}\KeywordTok{rnorm}\NormalTok{(}\DecValTok{1000000}\NormalTok{), }\DecValTok{6}\NormalTok{), }\DataTypeTok{nrow =} \DecValTok{6}\NormalTok{)}\OperatorTok{^}\DecValTok{2}
...
\NormalTok{  Q[}\DecValTok{1}\NormalTok{,] =}\StringTok{ }\NormalTok{X_squared[}\DecValTok{1}\NormalTok{,]}
\NormalTok{  Q[i,] =}\StringTok{ }\NormalTok{Q[(i}\DecValTok{-1}\NormalTok{),] }\OperatorTok{+}\StringTok{ }\NormalTok{X_squared[i,]}
\NormalTok{\}}

\KeywordTok{plot}\NormalTok{(}\OtherTok{NULL}\NormalTok{, }\DataTypeTok{xlim=}\KeywordTok{c}\NormalTok{(}\FloatTok{0.2}\NormalTok{,}\DecValTok{6}\NormalTok{), }\DataTypeTok{ylim =} \KeywordTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{,}\DecValTok{1}\NormalTok{),}
...
\includegraphics{R与tidyverse——数据分析入门_files/figure-latex/unnamed-chunk-60-1.pdf}
...
\hypertarget{dataframetibble}{%
\subsection{dataframe/tibble的概念}\label{dataframetibble}}
